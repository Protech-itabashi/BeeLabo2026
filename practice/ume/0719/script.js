    ...
    for (let i = 0; i < obstacles.length; i++){
        if (obstacles[i].x + obstacles[i].width < 0) {
            ...
            obstacles.push(new Obstacle(
                getRandomAribitrary(obstacles[obstacles.length-1].x + MIN_INTERVAL,obstacles[obstacles.length-1].x + MAX_INTERVAL),
                0,
                getRandomAribitrary(MIN_OBSTACLE_WIDTH,MAX_OBSTACLE_WIDTH),
                50,
            ));
        }
